...
- 全新 Material Design 界面，显示游戏 Build ID、支持版本及备份状态。
- 增加 Steam 验证恢复与二次确认的旧备份强制恢复；版本不匹配时保留恢复入口。
- Steam 验证后自动检测并刷新状态，无需反复手动点击。
- 修复原版模式切换弹窗、姓名及字形纵向位置；重制版对话人物名轻度加粗。
- 主程序与处理引擎各自单文件发布，依赖整合，无需另装 .NET 或 Python。

完整记录见 [CHANGELOG](CHANGELOG.md)。从旧版升级时，请完整解压新包后运行，避免混用旧引擎或旧数据。

| 项目 | 说明 |
| :--- | :--- |
| 工具版本 | **0.2.0** · 正式版；已通过用户本地安装、恢复及 Steam 验证后状态刷新测试 |
