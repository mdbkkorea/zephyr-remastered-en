...
import argparse,contextlib,gzip,hashlib,json,os,shutil,subprocess,sys,uuid
...
class Patcher:
 ...
 def status(self):
  self.guard_game();actual=self.actual();state=self.state();before={r['path']:r['before_sha256'] for r in self.files};after={r['path']:r['after_sha256'] for r in self.files}
  if self.journalfile.exists():kind='recovery_required'
  elif actual==before:kind='original'
  elif state and actual==state.get('installed_files'):
  ...
  return dict(status=kind,version=VERSION,installed_version=(state or {}).get('version'),game_build=self.patch['game_build'],can_install=kind in ['original','update_available','installed'],can_restore=bool(state) and kind in ['installed','update_available'],backup_available=self.original.exists())
 ...
 def _recover(self):
  ...
  if {r['path'] for r in entries}!=self.names:raise ValueError('恢复记录不匹配')
  for r in entries:
   p=safe(self.game,r['path'])
   if not p.exists() or sha(p) not in [r['before'],r['after']]:raise ValueError('操作中断后游戏文件又发生变化；为保护新版文件，未自动回滚。请联系维护者。')
   if sha(safe(rollback,r['path']))!=r['before']:raise ValueError('事务备份损坏')
  for r in entries:
   self.stopped();p=safe(self.game,r['path']);tmp=p.with_name(p.name+'.zephyr-patcher.tmp')
   if tmp.exists():tmp.unlink()
   if sha(p)!=r['before']:self.copy_checked(safe(rollback,r['path']),p,r['before'])
 ...
 def recover(self):
  ...
  return self.status()
 def transaction(self,source,desired,next_state,fail_after=None):
  self.stopped();before=self.actual();txid=uuid.uuid4().hex;rollback=self.home/'transactions'/txid/'rollback'
  entries=[dict(path=r['path'],before=before[r['path']],after=desired[r['path']]) for r in self.files]
  for r in entries:
  ...
  try:
   for i,r in enumerate(entries):
    ...
    if sha(p)!=r['before']:raise ValueError('安装期间游戏文件发生变化')
...
def main():
 ...
 ap=argparse.ArgumentParser();ap.add_argument('action',choices=['status','install','restore','recover','verify']);ap.add_argument('--game',required=True);ap.add_argument('--payload');args=ap.parse_args()
 try:
  p=Patcher(args.game,args.payload);result=getattr(p,args.action)();print(json.dumps(dict(ok=True,**result),ensure_ascii=False))
